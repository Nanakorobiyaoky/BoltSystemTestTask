...
describe('UsersService', () => {
  ...
  describe('create Author', () => {
    ...
    it('should throw Exception', async () => {
      await expect(usersService.saveUser({email: 'author@gmail.com', fullName: 'author', password: 'author' })).rejects.toThrowError(
  ...
  describe('create Admin', () => {
    ...
    it('should throw Exception', async () => {
      await expect(usersService.saveSystemUser({email: 'admin', name: 'admin', password: 'admin' })).rejects.toThrowError(
        new RpcException({ message: 'System user with this email/name already exists', status: HttpStatus.BAD_REQUEST })
      );
    });
  });
  ...
  describe('get all Client users', () => {
    it('should return AllUsersDto', async () => {
      ...
      for (let user of result.usersEditors) {
        expect(user instanceof ClientUserEntity).toBe(true)
      }
      for (let user of result.usersAuthors) {
        expect(user instanceof ClientUserEntity).toBe(true)
      }
    })
